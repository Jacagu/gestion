...
import { analyzeTimeSeries } from "@/lib/calculations/time-series-analysis";
import { TimeSeriesAnalysisResult } from "@/types/analysis-results";

export default function TimeSeriesAnalysisPage() {
  const [data, setData] = useState<number[]>([]);
  const [fileName, setFileName] = useState<string>("");
  const [results, setResults] = useState<TimeSeriesAnalysisResult | null>(null);
        ...
        try {
          ...
          if (file.name.endsWith(".xlsx") || file.name.endsWith(".xls")) {
            ...
            if (numericData.length === 0) {
              reject(
                new Error(
                  "No se encontraron datos numéricos en el archivo Excel",
                ),
              );
            }
          ...
          // Procesar como archivo de texto
            ...
            const numericData = lines
              ...
              .flatMap((line) => {
                ...
                return values
                  .filter((val) => !isNaN(Number(val)) && val.trim() !== "")
                  .map((val) => Number(val));
              });
            ...
            if (numericData.length === 0) {
              reject(
                new Error(
                  "No se encontraron datos numéricos en el archivo de texto",
                ),
              );
            }
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <p className="text-muted-foreground text-sm">
                Sube un archivo con los datos de la serie temporal (.xlsx, .csv,
                .txt)
              </p>
            </div>
            ...
            {fileName && !error && (
              <Alert
                variant="default"
                className="border-green-200 bg-green-50 text-green-800 dark:border-green-900 dark:bg-green-950/30 dark:text-green-400"
              >
            ...
            <Alert>
              ...
              <AlertDescription>
                Esta herramienta analiza series temporales, evalúa
                estacionariedad y realiza pronósticos utilizando suavización
                exponencial.
              </AlertDescription>
            </Alert>
          </div>
